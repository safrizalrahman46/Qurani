...
import io.qurani.android.databinding.DialogSelectionBinding
import io.qurani.android.manager.App
import io.qurani.android.manager.BuildVars
import io.qurani.android.manager.CountryUtils
import io.qurani.android.manager.adapter.BaseArrayAdapter
import io.qurani.android.manager.adapter.SelectionDialogRvAdapter
...
class SelectionDialog<T : SelectionItem> : io.qurani.android.manager.net.observer.NetworkObserverDialog(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        when {
            arguments?.getSerializable(io.qurani.android.manager.App.SELECTION_TYPE) == Selection.Country -> {
                setCountries()

            }
            arguments?.getSerializable(io.qurani.android.manager.App.SELECTION_TYPE) == Selection.Language -> {
    ...
    private fun setLngs() {
        ...
        mBinding.selectionProgressBar.visibility = View.GONE
        mBinding.selectionRv.visibility = View.VISIBLE
        val supportedLangs = io.qurani.android.manager.App.appConfig.supportedLangs
    ...
    private fun setCountries() {
        mBinding.selectionProgressBar.visibility = View.GONE
        mBinding.selectionRv.visibility = View.VISIBLE
        mCountries = ArrayList(io.qurani.android.manager.CountryUtils.getAllCountries(requireContext()))
        for (country in mCountries) {
            country.title = "${country.name} (${country.callingCode})"
            country.img = io.qurani.android.manager.CountryUtils.getFlagDrawableResId(country)
        }
        val adpt = SelectionDialogRvAdapter(io.qurani.android.manager.CountryUtils.getAllCountries(requireContext()))
        with(mBinding.selectionRv) {
            adapter = adpt
            addOnItemTouchListener(ItemClickListener(this, this@SelectionDialog))
        }
    ...
    override fun onClick(view: View?, position: Int, id: Int) {
        if (mItems != null) {
            ...
            mOnItemSelection.onItemSelected((mBinding.selectionRv.adapter as io.qurani.android.manager.adapter.BaseArrayAdapter<T, *>).items[position])
        }
        dismiss()
    }
