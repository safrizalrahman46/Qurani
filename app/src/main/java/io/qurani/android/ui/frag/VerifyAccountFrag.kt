...
import io.qurani.android.databinding.FragVerifyAccountBinding
...
import io.qurani.android.presenterImpl.VerifyAccountPresenterImpl
import io.qurani.android.ui.MainActivity
import io.qurani.android.ui.widget.LoadingDialog


class VerifyAccountFrag : io.qurani.android.manager.net.observer.NetworkObserverFragment(), View.OnClickListener {
    ...
    private fun init() {
        ...
        val parcelable = requireArguments().getParcelable<Parcelable>(io.qurani.android.manager.App.SIGN_UP)!!
    ...
    fun onCodeSent(response: io.qurani.android.model.BaseResponse) {
    ...
    fun onVerificationReceived(data: Data<User>) {
        if (data.isSuccessful || data.status == ResponseStatus.AUTH_GO_TO_STEP3.value()) {
            ...
            intent.putExtra(io.qurani.android.manager.App.USER_ID, mUserId)
