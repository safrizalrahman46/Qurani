...
import io.qurani.android.databinding.FragCommentDetailsBinding
...
import io.qurani.android.presenterImpl.CommentDetailsPresenterImpl
...
class CommentDetailsFrag : Fragment(), View.OnClickListener, ItemCallback<Comment> {
    ...
    private fun init() {
        ...
        mIsInstructor = requireArguments().getBoolean(io.qurani.android.manager.App.INSTRUCTOR_TYPE, false)
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.commentDetailsStudentHeader -> {
                val bundle = Bundle()
                bundle.putParcelable(io.qurani.android.manager.App.COURSE, mComment.course)
            ...
            R.id.commentDetailsReportBtn, R.id.commentDetailsReplyBtn -> {
                if (!mIsInstructor && v.id == R.id.commentDetailsReportBtn) {
                    ...
                    val bundle = Bundle()
                    bundle.putString(io.qurani.android.manager.App.TITLE, getString(R.string.delete))
                    bundle.putString(io.qurani.android.manager.App.TEXT, getString(R.string.delete_comment))
                    ...
                    val bundle = Bundle()
                    bundle.putSerializable(io.qurani.android.manager.App.SELECTION_TYPE, selectionType)
...
//            dialog.setOnCommentSavedListener(this)
    ...
    fun onCommentRemoved(rsp: io.qurani.android.model.BaseResponse) {
        if (rsp.isSuccessful) {
