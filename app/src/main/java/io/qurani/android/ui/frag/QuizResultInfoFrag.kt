...
import io.qurani.android.databinding.FragQuizResultBinding
...
import io.qurani.android.presenterImpl.QuizResultInfoPresenterImpl
...
class QuizResultInfoFrag : io.qurani.android.manager.net.observer.NetworkObserverFragment(), View.OnClickListener {
    ...
    private fun init() {
        ...
        val result = requireArguments().getParcelable<QuizResult>(io.qurani.android.manager.App.RESULT)
        val quiz = requireArguments().getParcelable<Quiz>(io.qurani.android.manager.App.QUIZ)
        val quizId = requireArguments().getInt(io.qurani.android.manager.App.ID)

        if (result != null) {
            (activity as MainActivity).showToolbar(toolbarOptions, R.string.quiz_result)

            mResult = result
            mIsInstructorReview = requireArguments().getBoolean(io.qurani.android.manager.App.INSTRUCTOR_TYPE, false)
    ...
    private fun goToReviewPage() {
        val bundle = Bundle()
        bundle.putParcelable(io.qurani.android.manager.App.RESULT, mResult)
        bundle.putBoolean(io.qurani.android.manager.App.INSTRUCTOR_TYPE, mIsInstructorReview)
    ...
    private fun showRetryConfirmation() {
        ...
        val bundle = Bundle()
        bundle.putString(io.qurani.android.manager.App.TITLE, getString(R.string.retry))
        bundle.putString(io.qurani.android.manager.App.TEXT, getString(R.string.retry_quiz_desc))
