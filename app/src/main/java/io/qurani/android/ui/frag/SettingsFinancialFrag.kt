...
import io.qurani.android.databinding.FragSettingsFinancialBinding
...
import io.qurani.android.presenterImpl.SettingsFinancialPresenterImpl
...
class SettingsFinancialFrag : Fragment(), View.OnClickListener, SettingsFrag.SaveCallback {
            ...
            registerForActivityResult(ResultContracts.SelectFile()) { uri ->
                if (uri != null) {
                    if (mCertStoragePermission) {
                        mCertFile = File(io.qurani.android.manager.UriToPath.getPath(requireContext(), uri))
                        mBinding.settingsFinancialCertFileTv.text = mCertFile!!.name
                        mBinding.settingsFinancialCertFileTv.visibility = View.VISIBLE
                    } else {
                        mIdScanFile = File(io.qurani.android.manager.UriToPath.getPath(requireContext(), uri))
    ...
    private fun initInfo() {
        ...
        if (io.qurani.android.manager.App.quickInfo != null && io.qurani.android.manager.App.quickInfo!!.financialApproval.toBoolean()) {
            val params =
...
//        mBinding.settingsFinancialAccountTypeSpinner.onItemSelectedListener = this
    ...
    fun onFilesSaved(response: io.qurani.android.model.BaseResponse) {
    ...
    private fun checkIfBothAreSaved(response: io.qurani.android.model.BaseResponse) {
