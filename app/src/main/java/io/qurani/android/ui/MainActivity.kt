...
import io.qurani.android.databinding.ActivityMainBinding
import io.qurani.android.manager.*
import io.qurani.android.manager.adapter.SlideMenuRvAdapter
...
import io.qurani.android.model.view.ViewShape
import io.qurani.android.presenter.Presenter
import io.qurani.android.presenterImpl.LogoutPresenterImpl
import io.qurani.android.ui.frag.*
import io.qurani.android.ui.widget.AppDialog
import io.qurani.android.ui.widget.CustomDuoDrawerLayout.LOCK_MODE_LOCKED_CLOSED
import io.qurani.android.ui.widget.CustomDuoDrawerLayout.LOCK_MODE_UNLOCKED
...
//        initBars()
    ...
    private fun initContracts() {
        ...
        mPaymentStatusContract =
            registerForActivityResult(ResultContracts.ActivityResultResponse()) { intent ->
                if (intent != null) {
                    val redirection =
                        intent.getParcelableExtra<PaymentRedirection>(io.qurani.android.manager.App.REDIRECTION)!!
    ...
    fun goToLoginPage(listener: OnRefreshListener? = null) {
        ...
        intent.putExtra(io.qurani.android.manager.App.REQUEDT_TO_LOGIN_FROM_INSIDE_APP, true)
    ...
    fun goToPaymentStatusPage(paymentStatus: Boolean, paymentRedirection: PaymentRedirection) {
        ...
        intent.putExtra(io.qurani.android.manager.App.PAYMENT_STATUS, paymentStatus)
        intent.putExtra(io.qurani.android.manager.App.REDIRECTION, paymentRedirection)
...
//    }
    ...
    private fun initNavDrawer() {
        ...
        items.add(MenuItem(R.drawable.bgdashboard, getString(R.string.home)))
        items.add(MenuItem(R.drawable.bgdashboardlayout, getString(R.string.dashboard)))
        items.add(MenuItem(R.drawable.bgclases, getString(R.string.classes)))
        items.add(MenuItem(R.drawable.bgmeeting, getString(R.string.meetings)))
        items.add(MenuItem(R.drawable.bgassignment, getString(R.string.assignments)))
        items.add(MenuItem(R.drawable.bgquiz, getString(R.string.quizzes)))
        items.add(MenuItem(R.drawable.bgcertificate, getString(R.string.certificates)))
        items.add(MenuItem(R.drawable.bgfavoritess, getString(R.string.favorites)))
        items.add(MenuItem(R.drawable.bgcomment, getString(R.string.comments)))
        items.add(MenuItem(R.drawable.bgfinancial, getString(R.string.financial)))
        items.add(MenuItem(R.drawable.bgsubscription, getString(R.string.subscription)))
        items.add(MenuItem(R.drawable.bgmail, getString(R.string.support)))
    ...
    private fun initNeedToRegister() {
        if (intent != null) {
            if (intent.getBooleanExtra(io.qurani.android.manager.App.SHOULD_REGISTER, false)) {
                showRegisterDialog(intent)
            }
        }
    }
    ...
    private fun showRegisterDialog(intent: Intent) {
        ...
        bundle.putParcelable(
            io.qurani.android.manager.App.USER, intent.getParcelableExtra<ThirdPartyLogin>(
                io.qurani.android.manager.App.USER))
        bundle.putInt(io.qurani.android.manager.App.USER_ID, intent.getIntExtra(io.qurani.android.manager.App.USER_ID, -1))
    ...
    private fun showLogoutConfirmation() {
        ...
        val bdl = Bundle()
        bdl.putString(io.qurani.android.manager.App.TITLE, getString(R.string.logout))
        bdl.putString(io.qurani.android.manager.App.TEXT, getString(R.string.logout_desc))
    ...
    private fun logout() {
        mPresenter.logout()
        io.qurani.android.manager.App.logout(this)
    }
...
//                mCurrentFrag?.view?.background = bg
    ...
    override fun onBackPressed() {
        ...
        if (io.qurani.android.manager.App.currentFrag != null && io.qurani.android.manager.App.currentFrag is QuizFrag) {
            (io.qurani.android.manager.App.currentFrag!! as QuizFrag).onBackPressed()
    ...
    override fun onClick(view: View?, position: Int, id: Int) {
        if (!io.qurani.android.manager.App.isLoggedIn() && position != SlideMenuItem.HOME.value()) {
            goToLoginPage(null)
            return
        }
...
//                R.anim.exit_to_bottom)
    ...
    fun showToolbar(toolbarOptions: ToolbarOptions, title: String) {
        ...
        if (toolbarOptions.endIcon != null) {
            ...
            if (toolbarOptions.endIcon == ToolbarOptions.Icon.CART && io.qurani.android.manager.App.quickInfo != null &&
                io.qurani.android.manager.App.quickInfo!!.cartItemsCount > 0
            ) {
                mBinding.mainToolbarEndCircleView.text = io.qurani.android.manager.App.quickInfo!!.cartItemsCount.toString()
    ...
    fun updateCart() {
        val quickInfo = io.qurani.android.manager.App.quickInfo
        if (quickInfo != null) {
