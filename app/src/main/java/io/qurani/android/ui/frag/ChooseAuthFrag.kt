...
import io.qurani.android.databinding.ChooseAuthFragBinding
...
class ChooseAuthFrag : Fragment(), View.OnClickListener {
    ...
    private fun init() {
        ...
        val signInRequest = arguments?.getBoolean(io.qurani.android.manager.App.REQUEDT_TO_LOGIN_FROM_INSIDE_APP, false)
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.skipLoginBtn -> {
                ...
                intent.putExtra(io.qurani.android.manager.App.IS_LOGIN, true)
    ...
    private fun signIn(signInRequest: Boolean = false) {
        val bundle = Bundle()
        bundle.putBoolean(io.qurani.android.manager.App.REQUEDT_TO_LOGIN_FROM_INSIDE_APP, signInRequest)
