...
import io.qurani.android.databinding.DialogNewTicketBinding
...
import io.qurani.android.presenterImpl.NewTicketPresenterImpl
import java.io.File
import java.io.Serializable


class NewTicketDialog : io.qurani.android.manager.net.observer.NetworkObserverBottomSheetDialog(), View.OnClickListener {
    ...
    private fun enableDisableBtn() {
        ...
        if (mTicketId > 0) {
            ...
            mBinding.ticketDialogSendBtn.isEnabled =
    ...
    private fun init() {
        ...
        if (requireArguments().getInt(io.qurani.android.manager.App.ID) > 0) {
            mTicketId = requireArguments().getInt(io.qurani.android.manager.App.ID)
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.ticketDialogSelectClassDepartmentEdtx -> {
                ...
                if (mType == Type.COURSE_SUPPORT) {
                    bundle.putString(io.qurani.android.manager.App.TITLE, getString(R.string.select_class))
                    ...
                    bundle.putString(io.qurani.android.manager.App.TITLE, getString(R.string.select_department))
    ...
    fun onTicketConversationSaved(conversation: Conversation, response: Data<io.qurani.android.model.Response>) {
        if (onSaved(response)) {
    ...
    fun onTicketConversationSaved(conversation: Conversation, response: io.qurani.android.model.BaseResponse) {
        if (onSaved(response)) {
            mOnTickeChatSaved?.onItem(conversation)
        }
    }

    fun onTicketSaved(ticket: Ticket, response: Data<io.qurani.android.model.Response>) {
    ...
    fun onTicketSaved(ticket: Ticket, response: io.qurani.android.model.BaseResponse) {
        ticket.createdAt = System.currentTimeMillis() / 1000
        if (onSaved(response)) {
            mOnTickeSaved?.onItem(ticket)
        }
    ...
    fun onSaved(response: io.qurani.android.model.BaseResponse): Boolean {
