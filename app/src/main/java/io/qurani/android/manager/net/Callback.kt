...
interface CustomCallback<T> : Callback<T> {
    ...
    fun onStateChanged(): io.qurani.android.manager.net.RetryListener?

    fun showNoNetworkPage(): Boolean {
        return true
    }

    private fun showNoNetwork(retryListener: io.qurani.android.manager.net.RetryListener?) {
        if (mErrorFrag == null || !ErrorFrag.isFragVisible) {
            ...
            io.qurani.android.manager.App.currentActivity.supportFragmentManager.beginTransaction()
