...
import io.qurani.android.databinding.ItemCourseBinding
...
class HomeClassRvAdapter(items: List<Course>, private val activity: MainActivity) :
    io.qurani.android.manager.adapter.BaseArrayAdapter<Course, HomeClassRvAdapter.ViewHolder>(items) {
        ...
        override fun onClick(v: View?) {
            when (v?.id) {
                ...
                R.id.itemCourseContainer -> {
                    val bundle = Bundle()
                    bundle.putParcelable(io.qurani.android.manager.App.COURSE, items[bindingAdapterPosition])
